...
> 📌 메모(2026-06-18): **애드센스 심사 종료·미신청 상태** → 이후 작업에서 '본문 대량수정 금지' 제약 **해제됨**.

- 2026-06-25 | [점검/발행] 6/25 축제 묶음 자동발행 점검 — fee 수정(commit 9a7a229) **첫 실발행 검증 PASS**. 8/8 표 요금이 원본 content 입장료와 정확 일치(40,000/77,000·55,000/무료 등), "요금 미공지"는 fee=null인 세미원 1곳뿐 → 6/18 글의 8/8 미공지 문제 실발행서 해소 확인. 도입부 '무료 N곳' 개수문장 0(개수산출 폐기 수정도 작동). [E] 전항목 PASS: 목요일 발행·sourceIds 8=본문 8(누수0)·직전제외(6/18)교집합0·sourceIds 유효·시기제목 4주(ceil 25/7)·상설 미혼입(최장 세미원 53일<90)·쿠팡0·요일주입 8/8·비수도권0·표무결성0·클리셰0. 차단급 0·수정 없음 | 점검만(코드변경0) | src/content/posts/2026-06-25-…festival.md(검증대상)
- 2026-06-18 | [SEO/404] GSC 404·색인누락 정리 후속 — blog 53건 생사판정 후 **생존글 301 1건만** 추가(commit 883032e): /blog/2026-05-14-seoul-youth-monthly-rent-support(슬래시 양변형) → /blog/2026-04-30-youth-rent-support/ 301(목적지 200·주제정확 검증, 체인0 단일홉 라이브확인). 나머지 삭제글은 404/301 유지(soft-404·와일드카드 회피). 백로그 2건 등재: §4-23 궁중문화축전(royal-culture) 가을 복원검토, §4-24 중랑장미축제 2027 시즌 신규글로 노출2971 회수. churn 상세·election은 404 정답이라 무시 | 배포됨 | public/_redirects, docs/project-guide.md(§4-23·24), commit 883032e
- 2026-06-18 | [SEO/기술] SEO 기술수정(commit f0a79c0): 슬래시 변형 7건 301 정합(hard-404 → /blog/ 301, 삭제글이라 트래픽 복구 아님·오류카운트 정리 목적), 블로그 canonical 슬래시 정합, robots 단일화, dead next-sitemap.config.js 제거. 본문 무수정. 라이브 검증 7건 301→/blog/ 200(체인0). election 64·삭제 블로그 56은 정책 보류(GSC export 대기) | 배포됨 | public/_redirects, src/app/blog/[slug]/page.tsx, src/app/robots.ts(삭제), next-sitemap.config.js(삭제), commit f0a79c0
- 2026-06-18 | [발행/검증] 축제 묶음 요금 정확화 발행(②) + 가드 3종 연쇄 합성검증(③) — ② generate-blog-post.js 한 파일(commit 9a7a229): extractFeeFromContent로 content '입장료' 행 파싱+normalizeFee(<br>·태그·개행·| 정리, 금액 보존), slim 9→10필드(fee) 주입·프롬프트 fee 그대로 표기/미공지만 폴백/임의생성 금지, 도입부 '무료 N곳' 개수문장 제거(freeCount=0 vs Gemini 독자산출 정합오류 차단)+죽은 freeCount/bundleFree 정리. 크롤(D)·번들(B)·표4컬럼·요일주입·직전제외·sourceIds·dedup·가드 무접촉. PREVIEW 2회 검증(fee 정확·<br>잔재0·개수문장0·회귀0), 다음 6/25 묶음부터 적용·6/18글 소급안됨. ③ 가드(validate-content.js) 합성검증: 미완 .md 2건(R1 헤딩절단·R2 산문절단) posts 주입→checkIncompleteEnding 단위 block 확인→validate --fix --quarantine 풀체인 실측(BLOCK→_quarantine 격리·quarantine-report.txt 기록·exit0·배포지속)·정상90글 오탐0/멱등. gh issue는 deploy.yml CI 셸스텝([ -s quarantine-report.txt ] 트리거)이라 조건충족+명령유효로 인과 정적확정(로컬 실생성 안함). tree baseline 원복. ③ 코드변경 0(검증만) | 배포됨(②) | scripts/generate-blog-post.js(②, 9a7a229), scripts/validate-content.js(③ 검증대상·무변경), docs/project-guide.md
- 2026-06-10 | [가이드/콘텐츠] /guides 3편 '자녀 적금·청약·보험 3축 가이드'(parenting-family-savings) — keyword-map §3 큐② C6(적금·청약·보험) 손글 플래그십. 3축(모으기=적금+증여 비과세 10년 2천만원/앞당기기=청약 만14세 5년·60회·월25만 인정/지키기=보험 구조 일반론) 비교표+깊이카드3+FAQ5(JSON-LD)+시점주의+발견CTA. 검증수치 전량 1차출처(국세청 증여재산공제·주택공급규칙 2024.9.30 개정·복지부 디딤씨앗통장), 디딤씨앗통장은 대상한정이라 안내카드·링크만, 보험절은 상품·요율 0 구조만. 제휴 미포함(정보 가이드). 1·2편 디자인·DepthCard 재사용(emerald 톤 추가), guides.ts 등재(허브 4·image 활성화). posts·pick-info 미경유(자동발행 충돌 0). 커밋 218f972 | 배포됨 | src/app/guides/parenting-family-savings/page.tsx, src/lib/guides.ts, public/images/og/parenting-family-savings.png, docs/project-guide.md
...
23. **궁중문화축전 글 복원 검토 — 신규(2026-06-18 등재)**. /blog/2026-royal-culture-festival-guide(및 2026-05-02
    변형)는 현재 삭제·만료(→/blog/ 301). **가을 궁중문화축전 시즌(9~10월) 재검색 수요** 가능 → 시즌 전 본문
    복원(또는 신규 작성) 검토. 지금은 복원 안 함·404(301) 유지. 애드센스 제약 해제로 본문 작성 자유.
24. **중랑장미축제 신규글로 노출 회수 — 신규(2026-06-18 등재)**. /blog/2026-04-28-seoul-rose-festival·2026-05-09-
    seoul-jungnang-rose-festival-guide(삭제, 노출 2971+977)는 현재 404 유지. 생존 장미축제 글은 부천(富川)뿐이라
    도시 불일치+시즌만료로 301 안 함. **2027년 4~5월 장미축제 시즌 전, 자동발행이 '중랑장미축제 2027' 신규글을
    생성하도록** 해 1위 검색어('중랑장미축제') 수요를 내년 시즌 신규글로 회수. (royal-culture 가을 백로그와 동일 처리)
