...
- **계산기 18종**: 연봉 실수령액(`salary`), 대출 이자(`loan`), 종합소득세(`income-tax`), 4대보험(`insurance`), 퇴직금(`retirement-pay`), 시급·주휴수당(`hourly-wage`), 자동차세(`car-tax`), 예금·적금 이자(`savings-interest`), 전월세 전환(`rent-conversion`), 부동산 중개보수(`brokerage-fee`), 연차수당(`annual-leave-allowance`), 해외주식 양도세(`overseas-stock-tax`), 기준 중위소득(`median-income`), 육아휴직급여(`parental-leave-pay`), 전기요금(`electricity-bill`), 실업급여(`unemployment-benefit`), K-패스 환급(`k-pass-refund`), 청년내일저축계좌 만기(`youth-savings-account`)
