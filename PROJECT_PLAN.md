# 프로젝트: 수도권 팁픽 (Tip-Pick)

## 목표
공공데이터포털(data.go.kr)에서 지역 행사/축제/지원금/기상 정보를 자동 수집하고,
...
## 페이지 구성
1. 메인 페이지: 수도권(인천, 경기, 서울)의 이번 달 행사/축제 카드 목록 + 수도권(인천, 경기, 서울)의 이번 달 지원금/혜택 정보 목록 + 개인 블로그 최근 게시글 3개 목록 표시 (날씨 위젯 제외)
2. 상세 페이지: 각 정보 클릭 시 상세 정보
3. 축제/행사 목록 페이지: 서울, 인천, 경기 실시간 날씨 위젯(Open-Meteo API) + **카드 클릭 시 7일 주간 예보 팝업(모달)** 제공 
4. 블로그 목록: AI가 자동 생성한 글 목록
5. 블로그 상세: 개별 블로그 글 읽기 (상단에 'TIP PICK' 라벨 표시)
